...
import { ArrowLeft, Save, Plus, Trash2, Sparkles, Layers } from 'lucide-react';
...
export default function AddProduct() {
  ...
  const handleSubmit = async (
    ...
    try {
      const res = (await addProduct(values))  as Product;
      
      if (res?._id) {
        navigate(`/dashboard/products/${res._id}/prices`);
      }
    } catch (error) {
      console.error("Core product profile establishment failed:", error);
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="flex items-center justify-between border-b border-zinc-100 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">Step 1: Establish core product specifications and taxonomy assets.</p>
          </div>
        </div>
        ...
        >
          <Save size={16} />
          <span>Save & Continue to Prices</span>
        </button>
      </div>
            ...
            <div className="space-y-6">
              <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-xs space-y-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <Field as="select" name="skinType" className="w-full bg-zinc-50 border border-zinc-200 rounded-lg px-3 py-2 text-sm focus:outline-hidden focus:border-emerald-500 focus:bg-white transition-all">
